...
interface UserType {
  uniqueId: number;
  name: string;
  // Add more user properties if available from the API
}
...
export default function Blog() {
  ...
  const getBlog = useCallback(async () => {
    try {
      const response = await API.get(`/blog/id/${uniqueId}`);
                ...
                <div className="blog">
                  ...
                  <div>
                    ...
                    {blog?.category?.map((item) => (
                      <p className="badge bg-danger-subtle text-dark me-1">{getCategoryToRelatedId(item)}</p>
                    ))}
                  </div>
                  <div>
                    <h6>Tags</h6>
                    {blog?.tags?.map((item) => (
                      <p className="badge bg-danger-subtle text-dark me-1">{getTagToRelatedId(item)}</p>
                    ))}
                  </div>
                </div>
              </div>
