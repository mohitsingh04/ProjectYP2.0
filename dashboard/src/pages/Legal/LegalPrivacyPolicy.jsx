...
export default function LegalPrivacyPolicy() {
  ...
  const getLegal = useCallback(async () => {
    try {
      ...
      setLegalPolicy(response.data.privacyPolicy);
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      <div className="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <div>
          <h1 className="page-title fw-semibold fs-20 mb-0">Privacy Policy</h1>
              ...
              {loading ? (
                ...
                <Form onSubmit={formik.handleSubmit}>
                  <Form.Group controlId="privacyPolicy">
                    <Form.Label>Enter Privacy Policy</Form.Label>
                    <JoditEditor
                      ref={editor}
                      value={formik.values.privacyPolicy}
                      onBlur={(newContent) =>
                        formik.setFieldValue("privacyPolicy", newContent)
                      }
                      onChange={(newContent) =>
                        formik.setFieldValue("privacyPolicy", newContent)
                      }
