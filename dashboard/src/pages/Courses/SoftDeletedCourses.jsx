...
export default function SoftDeletedCourses() {
  ...
  const restoreCourse = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Restored!",
          response?.data?.message || "Course has been restored.",
          "success"
        );
        getCourse();
      } catch (error) {
        Swal.fire(
          "Error",
          error.response?.data?.error || "Failed to restore course!",
          "error"
        );
      }
    }
