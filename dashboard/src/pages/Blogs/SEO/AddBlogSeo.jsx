...
export default function AddBlogSeo({ blog, getBlogSeo }) {
  ...
  useEffect(() => {
    if (blog?.blog) {
      const plainText = stripHtml(blog.blog);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await API.post(`/blog/seo`, values);
        Swal.fire({
          icon: "success",
          title: "Successfully",
